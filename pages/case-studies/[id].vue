...
const allCaseStudies = (Object.keys(caseStudiesData) as CaseStudyCategory[]).flatMap(
	(key) => caseStudiesData[key],
);
					...
					<div class="space-y-stack-md">
						<div class="flex items-center gap-4">
							<span class="font-label-caps text-outline uppercase">{{
								serviceLabel[caseStudy.serviceCategory] || caseStudy.serviceCategory
							}}</span>
							<span class="w-1.5 h-1.5 rounded-full bg-secondary shrink-0" />
							<span class="font-label-caps text-outline uppercase">{{ caseStudy.year }}</span>
						</div>
			...
			<UContainer class="mx-auto px-gutter">
				<div class="grid grid-cols-1 md:grid-cols-2 gap-stack-lg">
					<article class="space-y-3">
						<h2 class="font-headline-md text-primary-800">Výchozí situace</h2>
						<p class="font-body-md text-on-surface-variant">{{ caseStudy.detail.overview }}</p>
					</article>

					<article class="space-y-3">
						<h2 class="font-headline-md text-primary-800">Výzva</h2>
						<p class="font-body-md text-on-surface-variant">{{ caseStudy.detail.challenge }}</p>
					</article>

					<article class="space-y-3">
						<h2 class="font-headline-md text-primary-800">Řešení</h2>
						<p class="font-body-md text-on-surface-variant">{{ caseStudy.detail.solution }}</p>
					</article>

					<article class="space-y-3">
						<h2 class="font-headline-md text-primary-800">Výsledek</h2>
						<p class="font-body-md text-on-surface-variant">{{ caseStudy.detail.result }}</p>
					</article>
				</div>
			</UContainer>
		</section>
